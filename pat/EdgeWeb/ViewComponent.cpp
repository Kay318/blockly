...
float ViewComponent::GetZoomFactor()
{
    double fZoom;
    m_controller->get_ZoomFactor(&fZoom);
    return (float)fZoom;
}
...
HRESULT ViewComponent::SaveScreenshot(CString strName)
{
    HRESULT hr;
    wil::com_ptr<IStream> stream;
    hr = SHCreateStreamOnFileEx(strName, STGM_READWRITE | STGM_CREATE, FILE_ATTRIBUTE_NORMAL, TRUE, nullptr, &stream);
    if (FAILED(hr))
        return hr;
